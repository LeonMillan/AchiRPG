//=============================================================================
// ArchiRPG Notifications plugin v0.1.0
//=============================================================================

/*:
 * @plugindesc Adds default notifications handling with ArchiRPG.
 * @author LeonMillan
 * @orderAfter ArchiRPG
 */
...
(function (global, archipelago_js, MV) {
  ...
  var MV__namespace = /*#__PURE__*/_interopNamespaceDefault(MV);
  ...
  class Window_APToast extends MV__namespace.Window_Base {
    constructor() {
      super();
      _defineProperty(this, "timer", 0);
      _defineProperty(this, "renderedWidth", 0);
      _defineProperty(this, "toastPosition", NotificationsPosition.TopLeft);
  ...
  window.addEventListener('load', () => {
    ArchiRPG.client.addListener(archipelago_js.SERVER_PACKET_TYPE.PRINT_JSON, packet => {
  ...
  function getItemColor(item) {
    const isTrap = !!(item.flags & archipelago_js.ITEM_FLAGS.TRAP);
    return isTrap ? COLOR_INDEX.TrapName : COLOR_INDEX.ItemName;
  }
  function getItemIcon(item) {
    const isProgression = !!(item.flags & archipelago_js.ITEM_FLAGS.PROGRESSION);
    const isImportant = !!(item.flags & archipelago_js.ITEM_FLAGS.NEVER_EXCLUDE);
    const isTrap = !!(item.flags & archipelago_js.ITEM_FLAGS.TRAP);
  ...
  function handleUnlockedItem(item) {
    const isOwnItem = item.player === ArchiRPG.slot;
    const playerName = ArchiRPG.client.players.alias(item.player);
    ...
    const msgPlayer = `\\C[${playerColor}]${playerName}\\C[0]`;
    const msgItem = `\\C[${itemColor}]${itemName}\\C[0]`;
    const message = makeMessage(isOwnItem ? `You got ${msgItem}` : `You found ${msgItem} for ${msgPlayer}`, itemIcon);
    showNotification(message);
  }
  ...
  ArchiRPG.API.showUnlockedItems = function (items) {
    items.forEach(handleUnlockedItem);
  };
  ArchiRPG.API.showCustomMessage = function (message) {
    showNotification(message);
  };
